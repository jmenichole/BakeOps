...
| 4.6 | `updated_at` Triggers | Fixed | Created `010_ensure_updated_at_everywhere.sql` to apply triggers to all tables. |
| 3.3 | Widespread `any` types | In Progress | Hardened `database.ts`, `middleware.ts`, `ProductionPage`, and `SettingsPage`. |
...
| # | Issue | Target | Notes |
| --- | --- | --- | --- |
| 3.3 | Widespread `any` types | High | Systematic sweep of `/app/src` underway. 40% complete. |
| 4.6 | `updated_at` Triggers | Fixed | Triggers verified and automated for all schema tables. |
| 4.11 | Social Placeholders | Low | Update `SiteFooter.tsx` once brand handles are finalized. |
