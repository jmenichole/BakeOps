...
export default function DesignListPage() {
  ...
  const fetchDesigns = useCallback(async (isNextPage = false) => {
    try {
  ...
  const deleteDesign = async (id: string) => {
    ...
    try {
      ...
      const { error: deleteError } = await supabase
        ...
        .delete()
        .eq('id', id)
        .eq('baker_id', user.id); // Secure deletion
    ...
    <div className="animate-in fade-in duration-700 pb-20">
      ...
      {loading && page === 0 ? (
                ...
                <div className="aspect-square relative overflow-hidden bg-gray-50">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-6">
                    <div className="flex gap-2 w-full">
                      <button onClick={() => setSelectedDesign(design)} className="flex-1 btn glass text-secondary bg-white py-2 text-xs font-bold">View Detail</button>
                      <button onClick={() => deleteDesign(design.id)} className="p-2 bg-red-500 text-white rounded-xl hover:bg-red-600 transition-colors"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  </div>
                </div>
                <div className="p-6">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <span className="text-primary font-black text-sm">${Number(design.estimated_price).toFixed(2)}</span>
                  </div>
                  <p className="text-xs text-gray-400 line-clamp-2 leading-relaxed">{design.description || 'No description provided.'}</p>
                  <div className="mt-6 pt-4 border-t border-gray-50 flex justify-between items-center">
                    <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{new Date(design.created_at).toLocaleDateString()}</span>
          ...
          {hasMore && (
            <div className="mt-12 flex justify-center">
              <button
                ...
                className="btn btn-secondary px-10 py-4 flex items-center gap-3 text-sm font-bold"
              >
                {loadingMore ? <Loader2 className="w-5 h-5 animate-spin" /> : 'Load More Designs'}
              </button>
            </div>
          )}
        </>
        ...
        <div className="text-center py-20 card-bake bg-white border-dashed border-gray-200">
          <div className="w-20 h-20 bg-pink-50 text-primary rounded-full flex items-center justify-center mx-auto mb-6"><Search className="w-10 h-10" /></div>
          <h2 className="text-2xl font-serif font-black text-secondary mb-2">No results found</h2>
          <p className="text-gray-500 mb-10 max-w-sm mx-auto">No designs match &quot;{searchQuery}&quot;.</p>
          <button onClick={() => setSearchQuery('')} className="btn btn-primary px-8 py-4">Clear Search</button>
        </div>
      ) : (
        <div className="text-center py-20 card-bake bg-white border-dashed border-gray-200">
          <div className="w-20 h-20 bg-pink-50 text-primary rounded-full flex items-center justify-center mx-auto mb-6"><Plus className="w-10 h-10" /></div>
